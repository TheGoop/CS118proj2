...
class Packet {
    public:
        ...
        uint32_t getSeqNum() const { return m_header.seq_no; } 
        uint32_t getAckNum() const { return m_header.ack_no; } 
        ...
        int getSegmentSize() const { return getPayloadSize() + 12; } 

        void setSeqNum(uint16_t s) { m_header.seq_no = s; }
        void setAckNum(uint16_t a) { m_header.ack_no = a; }
        ...
        bool operator==(const Packet& other) const { return getSeqNum() == other.getSeqNum(); }
